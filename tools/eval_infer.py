...
def parse_args():
    parser = argparse.ArgumentParser(description='Run PointRCNN inference on every .bin in a pointcloud dataset and compute per-class metrics from matching label files.')
    parser.add_argument('--cfg_file', type=str, default='cfgs/default.yaml', help='Config file path.')
    parser.add_argument('--ckpt', type=str, default='../output/rcnn/default/ckpt/checkpoint_epoch_40.pth', help='Checkpoint to evaluate.')
    ...
    parser.add_argument('--set', dest='set_cfgs', default=None, nargs=argparse.REMAINDER,
                        help='Extra config keys, same format as other scripts.')
...
def main():
    ...
    if args.output_dir is not None:
        ...
        output_dir = (SCRIPT_DIR.parent / 'output' / 'infer_eval' / Path(args.cfg_file).stem / data_root.name).resolve()
